	...
	if (linkReq.readyState == '4'){
		...
		if(resposta != "0"){
			...
			for(var item in values){
				//alert(values[item]);
				if(values[item].indexOf("<meta") != 0 && values[item].indexOf("<style") != 0 && values[item].indexOf("\n") != 0){
...
function recebeAtividadeAction(){
	...
	if (linkReq.readyState == '4'){
		...
		for(var item in values){
				if(values[item].indexOf("<meta") != 0 && values[item].indexOf("<style") != 0 && values[item] != "" && values[item].indexOf("\n") != 0 )
					at.options[at.options.length] = new Option(values[item], values[item]);
		}
...
<form action="_next_step.php" method="POST" enctype="multipart/form-data">
	<?php
